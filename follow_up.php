...
<!DOCTYPE html>
    <head>
        ...
        <title>
            "NewLife Hospital-Book a follow_up"</title>
		    <link rel="stylesheet" href="style.css" />
    </head>
    <body>
        <h1>Booking a Follow-Up</h1>
        <h3>
        <?php
                ...
                try {
                    ...
                    if(CDP($id_result['dept_id'])){                                         #for cardiology department
                        ...
                        if($clash_result && $statement5->rowCount() > 0){                   #there may be clashes
                            ...
                            $get_duration = "SELECT * FROM appointment_info WHERE appointment_type = 'FOL04'";
                            ...
                            #all slots are free
                            $book_appointment = "SELECT duration, charges FROM appointment_info WHERE appointment_type = 'FOL04'";
                            ...
                            #$end_time = $end_time+(30*60);                                  #convert minutes to seconds then add to time
                            ...
                            #echo "<br>" . $timings_result['start_time'] . "-" . $end_time . "=" . $appointment_info['duration'];
                    ...
                    elseif(HDP($id_result['dept_id'])){                                     #for hematology department
                        ...
                        if($clash_result && $statement5->rowCount() > 0){                   #there may be clashes
                            ...
                            foreach($clash_result as $row){
                                #echo "<br>" . $row['start_time'] . " " . $row['end_time'] . " " . $row['app_day'];
                                $_SESSION['dates'][$row['app_day']] = $row['end_time'];     #update the array we will use to check availability
                            }
                            #foreach($_SESSION['dates'] as $day => $end){
                            #    echo "<br>" . $day . "= " . $end;
                            #}
